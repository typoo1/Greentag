...
def getVer():
    ...
    if not(version[:2] == "74" or version[:2] == "79"):
...
def Ping(name, txt, queue):
    print(name, file=txt)
    status = str(os.system("Echo off & ping -n 1 " + name + " > NUL"))
    if status == "0":
        status = "online"
...
def PrintOffline():
    ...
    for Reg in offlineReg:
        x = threading.Thread(target = Ping, args=(Reg.name, offTxt, offlineRemQueue))
        x.start()
        threads.append(x)
    for thread in threads:
        thread.join()
