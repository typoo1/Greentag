...
"""
TMPCus = ["Samuel Goldstein", "Tyrone Robinson", "Terrence Lattimore", "Sidra Davis"]
ORLCus = ["Samuel Goldstein", "Tyrone Robinson", "Terrence Lattimore", "Sidra Davis"]
SDOCus = ["Samuel Goldstein", "Tyrone Robinson", "Terrence Lattimore", "Sidra Davis"]
SATCus = ["Samuel Goldstein", "Tyrone Robinson", "Terrence Lattimore", "Sidra Davis"]
LAGCus = ["Samuel Goldstein", "Tyrone Robinson", "Terrence Lattimore", "Sidra Davis"]
WILCus = ["Samuel Goldstein", "Tyrone Robinson", "Terrence Lattimore", "Sidra Davis"]
...
def setPark(City):
    ...
    print("Attempting to set to city " + city)
    if(city == "TMP"): #Tampa
            Park1 = "BGT"
            Park2 = "AIT"
...
def fillForms():
    for reg in offlineReg:
        ...
        driver.set_page_load_timeout(15) #Set maximum time that webpage is left to load
        driver.get("https://sea.service-now.com/nav_to.do?uri=%2Fhome.do") #It is necissary to navigate to the Service now homepage to prevent a series of redirects which breaks the remainders of the script
        driver.get("https://sea.service-now.com/incident.do?sys_id=-1&sysparm_query=active=true&sysparm_stack=incident_list.do?sysparm_query=active=true") #Navigate to the New incident form.
        ...
        #GROUP.send_keys(r"Park1 + "-IT") #should be filling automatically based on CI
        #GROUP.send_keys("\ue004")
...
class Register:
    ...
    """
    Method that defins how to print information stored in the Register object
    """
    def printReg(self):
    ...
    if a>0:
        for message2 in reversed(messages):
            try:
                ...
                if sender != "":
                    if sender == "seap2018@seaworld.com" or "xstorereport@seaworld.com": #TODO find correct email addresses"
                        if sdate[:-15] == str(tarDate):
                            ...
                            while i < len(strings):
                                ...
                                if(re.search(Park2c, strings[i]) or re.search(Park2m, strings[i]) or re.search(Park1c, strings[i])
                                        or re.search(Park1m, strings[i])):
                                    z = i
...
def getConfig():
    ...
    if not (os.path.exists(path + "config.txt")):
        ...
        i = 0
        if(x == "TMP"):
            if(input("Would you like to use the default customers for your park?").lower() == "y"):
                if(city == "TMP"):
                    Cus = TMPCus
                if(city == "ORL"):
                    Cus = ORLCus
...
def getEmails(Account):
    for account in accounts:
        ...
        print("entered account for loop")
